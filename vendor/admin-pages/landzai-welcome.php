<?php
function landzai_let_to_num( $size ) {
  $l   = substr( $size, -1 );
  $ret = substr( $size, 0, -1 );
  switch ( strtoupper( $l ) ) {
    case 'P':
...
<div class="wrap about-wrap landzai-wrap">
    <h1><?php _e( 'Welcome to Landzai', 'landzai' ); ?></h1>

    <div class="about-text"><?php echo esc_html__( 'Landzai theme is now installed and ready to use!', 'landzai' ); ?></div>
<div class="landzai-badge">
    
    <p><?php echo esc_html($landzaitheme->get( 'Version' )); ?></p>
</div>
    <h2 class="nav-tab-wrapper">
        <?php
        printf( '<a href="#" class="nav-tab nav-tab-active">%s</a>', __( 'Welcome', 'landzai' ) );
        printf( '<a href="%s" class="nav-tab">%s</a>', admin_url( 'customize.php' ), __( 'Theme Options', 'landzai' ) );
    ...
    <div class="landzai-system-stats">
        <h3>System Status</h3>

    <table class="system-status-table">
        <tbody>
                     <tr>
							<td><?php esc_html_e( 'WP Version', 'landzai' ); ?></td>
							<td>
						...
						<tr>
							<td><?php esc_html_e( 'Language', 'landzai' ); ?></td>
							<td><?php echo get_locale() ?></td>
						</tr>
						
						<tr>
							<td><?php esc_html_e( 'WP Memory Limit', 'landzai' ); ?></td>
							<td><?php
								$memory = landzai_let_to_num( WP_MEMORY_LIMIT );

								if ( $memory < 100663296 ) {
									echo '<mark class="error">' . sprintf(esc_html__('%s - We recommend setting memory to at least 96MB. %s.','landzai'), size_format( $memory ), '<a href="' . esc_url('//www.wpbeginner.com/wp-tutorials/fix-wordpress-memory-exhausted-error-increase-php-memory/') . '" target="_blank">'.esc_html__('More info','landzai').'</a>') . '</mark>';
								} else {
						...
						<tr>
							<td><?php esc_html_e( 'PHP Max Input Vars', 'landzai' ); ?></td>
							<td><?php
								$max_input = ini_get('max_input_vars');
								if ( $max_input < 3000 ) {
									echo '<mark class="error">' . sprintf( wp_kses(__( '%s - We recommend setting PHP max_input_vars to at least 3000. See: <a href="%s" target="_blank">Increasing the PHP max vars limit</a>', 'landzai' ), array( 'a' => array( 'href' => array(),'target' => array() ) ) ), $max_input, '//teconce.com/support/2018/12/05/increasing-max-input-vars/' ) . '</mark>';
								} else {
						...
						<tr>
						  <td>
						     <?php esc_html_e( 'PHP Version', 'landzai' ); ?>
						  </td>
					...
					?>
						...
						<tr>
						    <td>
						     <?php esc_html_e( 'Server Info', 'landzai' ); ?>
						  </td>
						...
						<tr>
						    <td>
						        <?php esc_html_e( 'Secure Connection(HTTPS)', 'landzai' ); ?>
						    </td>
						    <td>
						        <?php 
    ...
    <table class="system-status-table">
        <tbody>
            <tr>
                <td><?php esc_html_e( 'Theme Name', 'landzai' ); ?></td>
                <td><?php echo wp_get_theme(); ?></td>
            </tr>
            
             <tr>
                <td><?php esc_html_e( 'Author Name', 'landzai' ); ?></td>
                <td><?php echo esc_html($landzaitheme->get( 'Author' )); ?></td>
            </tr>
            
            <tr>
					<td><?php esc_html_e( 'Current Version', 'landzai' ); ?></td>
					<td><?php echo esc_html($landzaitheme->get( 'Version' )); ?></td>
				</tr>
				
				  <tr>
					<td><?php esc_html_e( 'Text Domain', 'landzai' ); ?></td>
					<td><?php echo esc_html($landzaitheme->get( 'TextDomain' )); ?></td>
				</tr>
				
				<tr>
				    <td><?php esc_html_e( 'Child Theme', 'landzai' ); ?></td>
